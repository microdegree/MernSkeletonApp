...
router.post("/login", (request, response) => {
    ...
    database.collection("userTable").find({ "email": request.body.email, "password": request.body.password }).toArray((error, result) => {
        if (error) {
            return response.status(500).send(error);
        }
...
router.post("/getUserInfo", (request, response) => {
    ...
    database.collection("userTable").find({ "email": request.body.email }).toArray((error, result) => {
        if (error) {
            return response.status(500).send(error);
        }
